...
NMC_pre_clean <- function(NMC = NMC){
...
NMC_merge_cases_exact <- function(NMC1 = NMC1){
  # Exact stirng matching
  exact_match <- NMC1 %>%
    mutate(epidemiological_classification = factor( str_to_title(epidemiological_classification), levels = c(
      "Confirmed Case" , # make sure confirmed case is leveled first so that it is not discarded. 
      "Clinical Notification" ,  "Laboratory Notification" ,        "Discarded Case"    ,      "Probable Case" 
    )))%>%
      arrange(epidemiological_classification)%>%
  ...
  exact_match$epidemiological_classification%>%unique
  exact_match%>%group_by (UID)%>%filter(epidemiological_classification %in% "Confirmed Case")%>%view
  exact_match
  xtabs(~ dup_number, epidemiological_classification, data = exact_match)
  xtabs(~ duplicate, epidemiological_classification, data = exact_match)
  ...
  xtabs(~ dup_number, data = filtered_df)
  
  filtered_df$epidemiological_classification  %>%str_to_title%>%unique
  ...
  #filtered_df%>%view()
  merged_df%>%view()
  xtabs(~ epidemiological_classification, data = merged_df)
  xtabs(~ dup_number+epidemiological_classification, data = merged_df)
  merged_df%>%filter( epidemiological_classification == "Confirmed Case")%>%view()
  ...
  return(final_full_exact)
}
  
NMC_merge_cases_fuzzy <- function(NMC1 = NMC1){
  ######################################################################
  ##########
